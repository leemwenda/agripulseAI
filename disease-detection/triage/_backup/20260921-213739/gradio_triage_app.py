...
def verdict(probs, margin, classes, thresholds=None):
    """-> (kind, top_label): kind is 'uncertain', 'healthy' or 'disease'.

    Uses the same rule as training and evaluation (vision.decide): a disease is named only when the
    model is clearly ahead AND above both the app floor and that disease's calibrated threshold from
    the checkpoint. Checkpoints without thresholds behave as before (floor only)."""
    return get_vision().decide(probs, margin, thresholds or {}, DISEASE_CONFIDENCE_FLOOR)
...
def on_photo(img, manual):
    ...
    kind, top = verdict(res["probs"], res["margin"], res["classes"], res.get("thresholds"))
...
def diagnose_live(img, live_enabled):
    ...
    kind, top = verdict(averaged, res["margin"], res["classes"], res.get("thresholds"))
    if kind == "disease":
